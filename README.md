...
| [sqlhadr-review](#sqlhadr-review) | Analyze Always On AG health from `sys.dm_hadr_*` DMVs — 27 checks (H1–H27): replica connectivity, data loss risk, recovery time, throughput, configuration, modern AG features |
| [sqlag-review](#sqlag-review) | Audit Always On AG configuration correctness across all layers — 35 checks (F1–F35): prerequisites, replica design, listener architecture, backup strategy, endpoint security, distributed AG topology, Basic/Contained AG constraints |
...
Analyzes SQL Server instance and database configuration drift against proven DBA best practices. Applies 28 checks (B1–B28) across five categories: parallelism tuning, memory configuration, database-level settings, file and storage configuration, and surface area exposure.
...
| H1–H27 | 27 | sqlhadr-review | Always On AG health checks |
| F1–F35 | 35 | sqlag-review | AG configuration correctness: prerequisites, replica design, listener, backup, endpoint security, distributed AG, advanced features |
...
| B1–B28 | 28 | sqldbconfig-review | Configuration drift: MAXDOP/NUMA, CTP, Max Server Memory, auto-shrink, auto-close, compat level, RCSI, page verification, VLF count, percent growth, IFI, TempDB file count, surface area |
...
| Y1–Y14 | 14 | sqlmigration-review | Migration compatibility: edition-gated features, version/compat-level ceiling, collation, discontinued features, In-Memory OLTP, Azure SQL limits, backup/log-chain readiness, AG seeding limits, lifecycle |
