...
|---------------|----------------|
| **31 Tools** | `list_skills`, `get_skill`, `get_reference`, `route_artifact` (26 artifact types: `tsql`, `sqlplan`, `plancompare`, `planbatch`, `deadlock`, `blocking`, `waits`, `trace`, `stats`, `querystore`, `procstats`, `hadr`, `clusterlog`, `errorlog`, `spn`, `memory`, `diskio`, `encryption`, `dbconfig`, `setuplog`, `ssrslog`, `agconfig`, `migration`, `migrationsecurity`, `migrationobjects`, `mixed`), plus one tool per skill (×27) that accepts your artifact as `input` |
| **Resources** | `mssql://skills` index, `mssql://skills/{name}` (×27), `mssql://skills/{name}/references` index plus one resource per reference file, `mssql://guide`, `mssql://version-compat` |
...
║                                                              ║
║  Blocking chain DMVs / blocked process report XML            ║
║         │                                                    ║
║         ▼                                                    ║
║  /sqlblocking-review blocking_capture.txt                    ║
║  54 checks — head blocker and whether it self-resolves,      ║
║  chain depth/fan-out, escalation, Sch-M, isolation, ORM      ║
║  defaults, per-index history when the chain is gone, BPR     ║
║                                                              ║
...
║         ▼                                                    ║
║  /sqlspn-review                                                 ║
║  54 checks — MSSQLSvc SPN, AG listener, delegation,          ║
║  Entra ID hybrid, gMSA rollover, CNAME alias, linked         ║
║  server / SSISDB double-hop limits, clock skew, token        ║
║  size, encryption types, Linux keytabs, driver limits        ║
║                                                              ║
╚══════════════════════════════════════════════════════════════╝
```
...
Analyze a live SQL Server blocking chain: find the head blocker, decide whether the block will clear on its own, read the lock evidence, and name the transaction or isolation design fault behind it — and, when the incident is
already over, work the historical evidence that survives it. 54 checks (BL1–BL54).
...
- **Capture script (preferred):** `skills/sqlblocking-review/scripts/capture-blocking.sql`, run while the blocking is happening — twice, 30–60 seconds apart, if you can.

  ```
  sqlcmd -S <server> -E -i skills/sqlblocking-review/scripts/capture-blocking.sql -o blocking_capture_1.txt
  ```
- **Blocked process report:** off by default. Turn it on, then capture `blocked_process_report` in an Extended Events session:
  ...
  RECONFIGURE;
  ```
- **SSMS:** Reports → Standard Reports → *Activity - All Blocking Transactions*, or Activity Monitor's **Blocked By** column.
- **`sp_who2`** output (the `BlkBy` column) also works, with less detail.
- **Community tools:** `sp_WhoIsActive @find_block_leaders = 1`, `sp_BlitzWho`, `sp_BlitzFirst @SinceStartup = 1`, `sp_HumanEvents`/`sp_HumanEventsBlockViewer`, or a blocking-tree script — paste the output as-is.
- **After the fact:** run section 6 of the capture script for per-index lock hot spots, Query Store lock waits, and blocking counters. See [`references/community-practices.md`](skills/sqlblocking-review/references/community-practices.md) for the tooling map.
...
Or paste the DMV output, the blocked process report XML, or a description of the symptom.

### What It Checks

| Group | Checks | Covers |
|-------|--------|--------|
| Chain topology | BL1–BL7 | Head blocker identification, wait duration, chain depth, fan-out, cross-database chains, concurrency exhaustion, chronic recurrence |
| Head-blocker state | BL8–BL15 | Long-running query, sleeping session with an open transaction, orphaned transaction, rollback, client not fetching results, client/server distributed deadlock, non-lock wait at the head, maintenance work |
| Lock evidence | BL16–BL23 | Lock escalation, object-level X locks, `Sch-M` blocking, hot resources, key-range locks, conversion waits, application locks, lock footprint |
| Transaction and isolation | BL24–BL30 | Long open transactions, transactions across client round-trips, elevated isolation, implicit transactions, lock hints, RCSI candidates, version store side effects |
| Observability and platform | BL31–BL36 | Blocked process threshold, capture targets, escalation overrides, scan-driven footprints, ADR and optimized locking |
| Historical and aggregate evidence | BL37–BL42 | Instance-wide lock wait share, per-index lock hot spots and escalation attempts (`sys.dm_db_index_operational_stats`), Query Store lock waits, blocking performance counters, sampled blocking logs |
| Structural and engine-level causes | BL43–BL48 | A benign head blocker with a `Sch-M` request queued behind it, statistics updates, lock partitioning, unindexed foreign keys, triggers and cascades, writes that lock every row they read |
| Client, tooling, and platform | BL49–BL54 | ORM and driver transaction defaults, client timeout and retry policy, Azure SQL differences, readable-secondary redo blocking, commit acknowledgement waits, alerting and escalation path |

The first question the analysis answers is whether the head blocker is **sleeping with an open transaction** (BL9/BL10 — it will not clear; kill it, then fix the client's error handling) or **actively running** (BL8 — it clears when the query finishes; tune the query instead).

**Sample output:** See [`skills/sqlblocking-review/examples/blocking-analysis.md`](skills/sqlblocking-review/examples/blocking-analysis.md).
...
Analyze SQL Server SPN (Service Principal Name) configuration and Active Directory Kerberos delegation settings to diagnose authentication failures, NTLM fallback, and double-hop connectivity problems. Applies **54 checks** (K1–K54) across eleven categories: MSSQLSvc SPN presence (K1–K6), service account binding (K7–K11), AG listener and alias (K12–K16), configuration and permissions (K17–K20), Kerberos delegation (K21–K25), AD account sensitivity (K26–K30), Entra ID / hybrid and advanced scenarios (K31–K40), double-hop platform constraints (K41–K44), Kerberos environment prerequisites (K45–K48), SQL Server on Linux keytabs (K49–K51), and client driver and service state (K52–K54).
...
**Entra ID / Hybrid and Advanced (K31–K40):** Entra ID hybrid join SPN gap (K31), Entra-only auth with orphaned AD SPN (K32), Azure SQL MI Windows Authentication flow not configured (K33), gMSA password rollover SPN drift (K34), FCI node-specific SPN leak (K35), distributed AG forwarder listener SPN (K36), `TrustedToAuthForDelegation` set for an RBCD path (K37), encryption type mismatch between account and KDC policy (K38), Write-SPN blocked by AdminSDHolder (K39), DNS CNAME alias without SPN (K40).

**Double-Hop Platform Constraints (K41–K44):** Linked server delegation path relies on RBCD, which linked servers do not support (K41); linked server constrained delegation below SQL 2017 CU17 (K42); SSISDB package double-hop under constrained delegation, which SSISDB does not support (K43); named instance dynamic port prevents a stable SPN (K44).

**Kerberos Environment Prerequisites (K45–K48):** Clock skew beyond the 5-minute tolerance (K45), Kerberos token size exceeded — SQL error 17832 (K46), RC4-only encryption types under AES enforcement (K47), client and server across a forest boundary (K48).

**SQL Server on Linux (K49–K51):** Keytab not configured in `mssql-conf` (K49), keytab encryption types mismatch the AD account (K50), keytab file ownership or permissions wrong (K51).

**Client Driver and Service State (K52–K54):** Legacy provider cannot use Kerberos over named pipes (K52), service account password changed without a service restart (K53), report server missing `RSWindowsNegotiate` (K54).
...
|--------|---------|-------------|
| `skills/sqlprocstats-review/scripts/collection/` | Persistent collectors — schema, tables, SPs, Agent job, PowerShell deployment | `/sqlprocstats-review`, `/sqlwait-review` |
...
| `skills/sqldeadlock-review/scripts/` | system_health reader + dedicated XE session for deadlock capture | `/sqldeadlock-review` |
| `skills/sqlquerystore-review/scripts/` | Queries A–D for Query Store analysis | `/sqlquerystore-review` |
...
**Checks:** 25 total — R1–R5 (top consumers), R6–R10 (per-execution efficiency),
R11–R15 (pattern detection), R16–R20 (trend analysis, requires ≥ 3 snapshots),
R21–R25 (advanced runtime patterns: natively compiled, CLR, trigger dominance, parallel-to-serial, QS instability).
...
**Sample output:** See [`skills/sqlprocstats-review/examples/proc_stats_output-analysis.md`](skills/sqlprocstats-review/examples/proc_stats_output-analysis.md).
...
**Sample output:** See [`skills/sqlencryption-review/examples/encryption-audit-analysis.md`](skills/sqlencryption-review/examples/encryption-audit-analysis.md).
...
**Sample output:** See [`skills/sqldbconfig-review/examples/config-drift-analysis.md`](skills/sqldbconfig-review/examples/config-drift-analysis.md).
...
**Reference script:** [`skills/sqlbootstraplog-review/scripts/check-pending-reboot.ps1`](skills/sqlbootstraplog-review/scripts/check-pending-reboot.ps1) — pre-flight pending-reboot gate for installs and patches (exit 0 = clear, 1 = reboot pending).
...
**Sample output:** See [`skills/sqlbootstraplog-review/examples/setup_summary-analysis.md`](skills/sqlbootstraplog-review/examples/setup_summary-analysis.md).
...
**Checks:** 24 total —
- G1–G4 Trace configuration and log health: `DefaultTraceSwitch` level, file size/retention, restart/recycle frequency from trace file rollover, stale component-level trace overrides
- G5–G9 Startup and report server database connectivity: `rsReportServerDatabaseUnavailable`, `rsReportServerDatabaseLogonFailed`, `rsErrorOpeningConnection` sub-causes, orphaned database pointers, `rsServerConfigurationError`
- G10–G13 Memory pressure and AppDomain recycling: `MemorySafetyMargin`/`MemoryThreshold`/`WorkingSetMaximum` configuration, hard recycles from memory allocation failures, `RecycleTime`/`MaxAppDomainUnloadTime` tuning
- G14–G18 Report processing and rendering: `ExecutionLog3` time-phase breakdown, legacy processing engine usage, `rsProcessingAborted` and error statuses, external image fetch latency
- G19–G21 Subscriptions and delivery: file share and email delivery extension failures, subscription scheduling clustering
- G22–G24 Scale-out and encryption keys: `rsInvalidReportServerDatabase` after upgrade, `rskeymgmt -j` join failures, missing symmetric key backups

**Reference script:** [`skills/ssrstracelog-review/scripts/collect-ssrs-diagnostics.ps1`](skills/ssrstracelog-review/scripts/collect-ssrs-diagnostics.ps1) — collects trace configuration, server configuration, trace log rollover counts, recent ERROR/Exception lines, and related Event Log entries in one pass.
...
**Sample output:** See [`skills/ssrstracelog-review/examples/ssrs_trace-analysis.md`](skills/ssrstracelog-review/examples/ssrs_trace-analysis.md).
...
**Sample output:** See [`skills/sqlmigration-review/examples/migration-facts-analysis.md`](skills/sqlmigration-review/examples/migration-facts-analysis.md).
...
**Input:** Pasted login/permission facts, output from [`skills/sqlmigration-security-review/scripts/capture-security-facts.sql`](skills/sqlmigration-security-review/scripts/capture-security-facts.sql), a natural-language description, or a file path to exported artifacts.
...
- J10–J12 Credentials & secrets: `sys.credentials` secret cannot be scripted and must be re-entered, proxy created before its backing credential, linked server stored login not migrated
- J13–J14 Certificates & keys: certificate-backed object migrated without its certificate, database master key not backed up before migration
...
**Sample output:** See [`skills/sqlmigration-security-review/examples/security-facts-analysis.md`](skills/sqlmigration-security-review/examples/security-facts-analysis.md).
...
**Input:** Pasted instance-object facts, output from [`skills/sqlmigration-objects-review/scripts/capture-objects-facts.sql`](skills/sqlmigration-objects-review/scripts/capture-objects-facts.sql), a natural-language description, or a file path to exported artifacts.
...
- M7–M9 Linked servers: provider not installed on target, network path unreachable from new location, `collation_compatible` setting drift
- M10–M11 Database Mail: profile/account not migrated, SMTP relay allow-list excludes target's outbound IP
- M12 Backup infrastructure: backup device path unreachable on target
- M13 Custom errors: `sys.messages` entry not migrated before a dependent alert relies on it
- M14 Server triggers: DDL/logon trigger definition not migrated
...
**Sample output:** See [`skills/sqlmigration-objects-review/examples/objects-facts-analysis.md`](skills/sqlmigration-objects-review/examples/objects-facts-analysis.md).
...
| BL1–BL54 | 54 | sqlblocking-review | Blocking chain: head blocker identification and state classification, chain topology, lock escalation/Sch-M/key-range evidence, transaction and isolation design, blocked process report readiness |
...
| K1–K54 | 54 | sqlspn-review | SPN and Kerberos delegation analysis |
