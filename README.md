...
| [sqlwait-review](#sqlwait-review) | Analyze `sys.dm_os_wait_stats` — 43 checks (V1–V43) across I/O, lock, parallelism, memory, CPU, latch contention, log space, poison/throttle waits, backup I/O, insert hotspots, trend analysis, modern feature waits (In-Memory OLTP, Columnstore, Query Store, Transaction/DTC, Service Broker, Full Text, Parallel Redo), forced memory grants, grant timeouts, stolen memory, file I/O latency, and IQP/PSP/ADR feature waits |
| [sqlplan-review](#sqlplan-review) | Analyze a single `.sqlplan` file — 107 checks, prioritized report |
...
| [query-store-review](#query-store-review) | Analyze Query Store DMV output — 30 checks for regressed queries, plan instability, resource hotspots, configuration health, and SQL 2019/2022 IQP/PSP/DOP/CE feedback |
| [procstats-review](#procstats-review) | Analyze collected `sys.dm_exec_procedure_stats` snapshots — 25 checks (R1–R25) for top consumers, per-execution efficiency, N+1 patterns, trends, and advanced runtime patterns |
| [clusterlog-review](#clusterlog-review) | Analyze WSFC `CLUSTER.LOG` for Always On AG root-cause diagnosis — 30 checks (L1–L30): lease timeouts, health check failures, quorum loss, node eviction, network partition, RHS crashes, Cloud Witness, Azure Arc, Contained AG |
| [hadr-health-review](#hadr-health-review) | Analyze Always On AG health from `sys.dm_hadr_*` DMVs — 27 checks (H1–H27): replica connectivity, data loss risk, recovery time, throughput, configuration, modern AG features |
| [errorlog-review](#errorlog-review) | Analyze SQL Server ERRORLOG for operational issues — 33 checks (E1–E33): AG failover events, lease expiry, memory pressure, I/O slow, corruption warnings, login failure bursts, startup/shutdown, SQL 2019/2022 feature events |
| [spn-review](#spn-review) | Analyze SQL Server SPN configuration and Kerberos delegation settings — 40 checks (K1–K40): SPN presence, service account binding, AG listener and alias, permissions, delegation, AD account sensitivity, Azure AD hybrid, gMSA rollover, CNAME alias |
...
║         ▼                                                    ║
║  /sqlwait-review                                             ║
║  43 checks — I/O, locks, parallelism, memory grants,         ║
...
║         ▼                                                    ║
║  /query-store-review                                         ║
║  30 checks — regressed queries, plan instability,            ║
...
Analyze a single execution plan for performance anti-patterns. Applies **107 checks** (S1–S36 statement-level, N1–N71 node-level) and produces a prioritized Critical → Warning → Info report with actionable fixes.
...
Batch-analyze a folder of `.sqlplan` files. Applies all 107 checks to each plan and produces a single aggregated `batch-analysis.md` dashboard.
...
Analyze SQL Server Query Store data to identify regressed queries, plan instability, top resource consumers, query-level wait patterns, configuration issues, and SQL 2019/2022 IQP/PSP/DOP/CE feedback signals. Applies **30 checks** (Q1–Q30) and maps every finding to companion skills for deep-dive analysis. Requires SQL Server 2016+.
...
in `skills/procstats-review/scripts/collection/`. Applies 25 checks (R1–R25) across five categories.
...
| N1–N71 | 71 | sqlplan-review | Execution plan — node-level |
| C1–C20 | 20 | sqlplan-compare | Plan regression checks |
| D1–D8 | 8 | sqlplan-index-advisor | Index derivation rules |
| P1–P16 | 16 | sqlplan-deadlock | Deadlock patterns |
| Q1–Q30 | 30 | query-store-review | Query Store health + regressions + IQP/PSP/CE/DOP feedback |
| R1–R25 | 25 | procstats-review | Procedure/trigger/function stats |
| L1–L30 | 30 | clusterlog-review | WSFC cluster log analysis |
| H1–H27 | 27 | hadr-health-review | Always On AG health checks |
| E1–E33 | 33 | errorlog-review | ERRORLOG operational analysis |
| K1–K40 | 40 | spn-review | SPN and Kerberos delegation analysis |
