...
| [sqlencryption-review](#sqlencryption-review) | Analyze the full SQL Server encryption posture from DMV output — 80 checks (A1–A80) across 15 categories: TDE, Always Encrypted, Cell-Level Encryption, Backup Encryption, Transport/TLS, TLS hardening, Certificate Management, Asymmetric & Symmetric Keys, Key Hierarchy, EKM/AKV, Compliance, AE advanced, operational key lifecycle, Ledger, Azure-specific |
...
Analyzes the complete SQL Server encryption posture by reviewing output from 17 DMVs covering all encryption layers. Applies 80 checks (A1–A80) across 15 categories: TDE, Always Encrypted, Cell-Level Encryption, Backup Encryption, Transport/TLS, TLS Hardening, Certificate Management, Asymmetric & Symmetric Keys, Key Hierarchy, EKM/AKV, Compliance, AE Advanced, Operational Key Lifecycle, Ledger, and Azure-Specific.

**Triggers:** `/sqlencryption-review`, `/encryption-review`, `/tde-review`, `/always-encrypted-review`, `/backup-encryption-review`, `/tls-review`, `/ledger-review`
...
- A57–A61: TLS Hardening (TLS 1.0/1.1 registry state, cipher suite audit, certificate CN/SAN mismatch, weak cipher detection, channel binding enforcement)
- A62–A66: AE Advanced (enclave attestation URL validation, secure enclave session status, HGS health, rich query support gaps, comparison/deterministic misuse for JOIN/GROUP BY)
- A67–A71: Operational Key Lifecycle (certificate near-expiry with no replacement, stale symmetric/asymmetric keys not rotated, DMK/SMK rotation gap, stale backup certificates, certificate cleanup backlog)
- A72–A76: Ledger (database ledger enabled/disabled, table ledger verification state, digest management — storage path, upload schedule gaps, digest verification failures)
- A77–A80: Azure-Specific (AKV with Managed HSM usage, SQL DB/Managed Instance TDE discrepancy, Azure AD token-based auth for AKV, Azure Arc enabled SQL encryption gaps)
...
| A1–A80 | 80 | sqlencryption-review | TDE, Always Encrypted, CLE, backup, transport, TLS hardening, certs, keys, EKM, compliance, AE advanced, operational key lifecycle, Ledger, Azure-specific |
