...
| [sqlhadr-review](#sqlhadr-review) | Analyze Always On AG health from `sys.dm_hadr_*` DMVs — 28 checks (H1–H28): replica connectivity, data loss risk, recovery time, throughput, configuration, modern AG features, seeding/initialization integrity |
...
Analyzes `sys.dm_hadr_*` DMV output to assess Always On Availability Group replica health: synchronization state, secondary lag, redo/log send queue sizes, and configuration gaps. Applies 28 checks (H1–H28) across six categories.
...
| H1–H28 | 28 | sqlhadr-review | Always On AG health checks, including seeding/initialization integrity |
