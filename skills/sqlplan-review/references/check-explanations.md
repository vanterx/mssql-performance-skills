...
> **[Unverified]** The `QueryTimeStats/@UdfCpuTime` and `@UdfElapsedTime` attribute names and their minimum SQL Server version have not been confirmed against Microsoft Learn. Validate before relying on the exact attribute names in production tooling.
...
> **[Unverified]** The `<WaitStats>` element's schema and minimum SQL Server version have not been confirmed against Microsoft Learn. Validate before relying on the exact element/attribute names in production tooling.
...
> **[Unverified]** The exact row-size estimator behavior for LOB/`(MAX)` columns (whether the flat estimate is still applied, and its precise figure, across current SQL Server/Azure SQL versions) has not been confirmed against Microsoft Learn. The mechanism below is stated qualitatively rather than as a specific byte figure until validated.
