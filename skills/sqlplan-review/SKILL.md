...
### S37 — Hidden Scalar UDF Time [Unverified — `QueryTimeStats/@UdfCpuTime` and `@UdfElapsedTime` attribute names and minimum SQL Server version not confirmed against Microsoft Learn]
- **Trigger:** `QueryTimeStats/@UdfCpuTime` > 0 or `QueryTimeStats/@UdfElapsedTime` > 0 on `StmtSimple`, with no N25 (visible scalar UDF operator) finding anywhere in the same statement
...
### S38 — In-Plan Wait Statistics Present [Unverified — `<WaitStats>` element schema and minimum SQL Server version not confirmed against Microsoft Learn]
- **Trigger:** `<WaitStats>` element present under `StmtSimple` (actual execution plan only) with any `Wait/@WaitTimeMs` ≥ 10% of the statement's total elapsed time
...
### N73 — Memory Grant Undersized by LOB/(MAX) Column Estimate [Unverified — exact row-size estimator behavior for LOB/(MAX) columns not confirmed against Microsoft Learn]
