...
- **Fix by wait type:**
  ...
  - `LOG_RATE_GOVERNOR` / `POOL_LOG_RATE_GOVERNOR` / `INSTANCE_LOG_RATE_GOVERNOR` (SQL Server 2016+; primarily observable in Azure SQL / Managed Instance where log generation rate is tier-enforced; on-premises SQL 2016–2019 these are internal waits that may appear in AG high-log-rate scenarios) — transaction log generation rate is being actively throttled. Reduce write volume, optimize large DML, check secondary replica health and tier limits.
