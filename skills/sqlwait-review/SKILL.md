...
- **Fix by wait type:**
  ...
  - `SE_REPL_*` (Azure SQL Database [Unverified]) — secondary replica replication throttle. The primary is generating logs faster than the secondary can apply them. Check `sys.dm_hadr_database_replica_states` for `redo_queue_size` and `redo_rate` on the lagging secondary.
