...
Plain-English explanations of every check in `sqlclusterlog-review`. Each entry follows the
...
2. Correlate with `/sqlhadr-review` H23 (contained system database check) and `/sqlerrorlog-review` E15–E19 (I/O error checks) for the underlying cause.
...
2. `io_subsystem` WARNING/ERROR → run `/sqlerrorlog-review` checks E15–E19 (I/O slow, stalled scheduler); measure disk latency with `sys.dm_io_virtual_file_stats`.
3. `resource` WARNING/ERROR → run `/sqlerrorlog-review` checks E9–E14 (memory pressure); check `sys.dm_os_memory_clerks` for clerks consuming abnormal page counts.
