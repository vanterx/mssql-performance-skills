...
> **Input:** `skills/sqlprocstats-review/examples/proc_stats_output.txt` — Q1 (Top CPU Consumers)
> Run with: `/sqlprocstats-review skills/sqlprocstats-review/examples/proc_stats_output.txt`
...
- **Fix:** Capture the actual execution plan immediately — this plan has been in cache for only 22 minutes, so it is fresh. Run `/sqlplan-review` on it. With `avg_logical_reads = 412,000` (see C2) the dominant cost is almost certainly a full scan. Run `/sqlindex-advisor` to generate covering index DDL.
...
- **Fix:** Run `/sqlindex-advisor` to generate covering indexes. With `physical_reads_delta = 84,000` over 18 executions = 4,667 physical reads/execution, approximately 1.1% cache miss rate — acceptable, but the volume means even 1% = significant disk I/O.
...
> **Note:** R16–R20 (trend checks) cannot be evaluated — Q5 trend data was not provided. Run Q5 from `04_report_queries.sql` and re-run `/sqlprocstats-review` with that output to assess worsening trends.
