...
| [`sqlencryption-review`](#sqlencryption-review) | `/sqlencryption-review`, `/tde-review`, `/encryption-review`, `/tls-review`, `/ledger-review` | `sys.databases`, `sys.dm_database_encryption_keys`, `sys.certificates`, `sys.symmetric_keys`, `sys.columns`, `msdb.dbo.backupset`, `sys.dm_exec_connections`, and related DMV output | Full encryption posture analysis — 80 checks (A1–A80) across 15 categories: TDE coverage, Always Encrypted, cell-level encryption, backup encryption, transport/TLS, certificate lifecycle, asymmetric/symmetric key management, DMK/SMK key hierarchy, EKM/AKV, compliance, TLS/network hardening, Always Encrypted advanced/enclave, operational key lifecycle, SQL Ledger, Azure encryption |
...
| `A1–A80` | `sqlencryption-review` | Encryption posture: TDE state, encryption scan, TDE cert backup/expiry, DEK algorithm, cert sharing, Always Encrypted column gaps, CMK store, CEK rotation, deterministic vs randomized, enclave, CLE deprecated algorithms, open key scope, password-only key protection, key age, backup encryption, backup cert backup/expiry, transport TLS, live unencrypted sessions, self-signed cert, cert private key, Service Broker/AG endpoint cert age, cert-based logins, SHA1/MD5 hash, DMK/SMK backup, DMK auto-decryption, linked server encryption, EKM provider, AKV rotation, FIPS compliance, sensitivity classification gaps, SQL Audit, TLS version/legacy cipher hardening, AE enclave attestation/driver compat, operational key lifecycle/archival, SQL Ledger enablement/digest storage, Azure TDE protector region/double encryption | 80 |

**Total: 637 checks across all skills.**
