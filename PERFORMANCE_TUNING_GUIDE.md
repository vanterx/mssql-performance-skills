...
| [`sqlencryption-review`](#sqlencryption-review) | `/sqlencryption-review`, `/tde-review`, `/encryption-review` | `sys.databases`, `sys.dm_database_encryption_keys`, `sys.certificates`, `sys.symmetric_keys`, `sys.columns`, `msdb.dbo.backupset`, `sys.dm_exec_connections`, and related DMV output | Full encryption posture analysis — 56 checks (A1–A56): TDE coverage, cert expiry/backup, Always Encrypted gaps, backup encryption, transport/TLS, certificate lifecycle, CLE key algorithms, DMK/SMK key hierarchy, EKM/AKV, FIPS/PCI-DSS/HIPAA/GDPR compliance |
...
1. **`/sqlencryption-review`** — collect output from the 7–17 recommended DMV queries in the skill prompt and paste. The skill runs 56 checks across all encryption layers: TDE coverage (A1–A8), Always Encrypted gaps (A9–A16), CLE key algorithm strength (A17–A21), backup encryption (A22–A25), transport/TLS (A26–A30), certificate lifecycle (A31–A38), key hierarchy (A44–A48), and compliance coverage (A53–A56). It outputs a coverage summary, ranked findings table, root-cause analysis, and compliance gap report against PCI-DSS v4/HIPAA/GDPR.
...
| `A1–A56` | `sqlencryption-review` | Encryption posture: TDE state, encryption scan, TDE cert backup/expiry, DEK algorithm, cert sharing, Always Encrypted column gaps, CMK store, CEK rotation, deterministic vs randomized, enclave, CLE deprecated algorithms, open key scope, password-only key protection, key age, backup encryption, backup cert backup/expiry, transport TLS, live unencrypted sessions, self-signed cert, cert private key, Service Broker/AG endpoint cert age, cert-based logins, SHA1/MD5 hash, DMK/SMK backup, DMK auto-decryption, linked server encryption, EKM provider, AKV rotation, FIPS compliance, sensitivity classification gaps, SQL Audit | 56 |

**Total: 613 checks across all skills.**
