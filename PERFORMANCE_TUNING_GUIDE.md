...
| `B1–B29` | `sqldbconfig-review` | Configuration drift: MAXDOP vs. NUMA topology, Cost Threshold for Parallelism, Optimize for Ad Hoc Workloads, Query Governor, Max Server Memory, Min Server Memory, LPIM, AWE, auto-shrink, auto-close, compatibility level, RCSI, page verification, auto-create/update statistics, Trustworthy, cross-DB chaining, VLF count, percent auto-growth (log and data), Instant File Initialization, TempDB file count, CLR, OLE Automation, Ad Hoc Distributed Queries, instance chaining, remote admin connection, service-SID sysadmin membership | 29 |
