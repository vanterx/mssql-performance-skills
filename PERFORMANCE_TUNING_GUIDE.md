...
| [`sqlag-review`](#sqlag-review) | `/sqlag-review` / `/ag-review` / `/ag-config-review` | `sys.availability_groups`, `sys.availability_replicas`, `sys.availability_group_listeners`, `sys.availability_group_listener_ip_addresses`, `sys.database_mirroring_endpoints`, `sys.databases` (AG members), `sys.certificates` output | AG configuration audit — 36 checks (F1–F36): prerequisites and instance setup, replica design, listener and network architecture, backup strategy, endpoint security, distributed AG and advanced features (Basic AG, Contained AG), operational monitoring including AG scale ceiling |
...
| [`sqlmigration-review`](#sqlmigration-review) | `/sqlmigration-review`, `/migration-review`, `/sql-migration-readiness` | Source/target server facts pasted as text, capture script output, natural-language description, or directory of exported artifacts | Migration version/edition/platform compatibility audit — 15 checks (Y1–Y15): edition-gated features, version/compatibility-level ceiling, collation, discontinued features, In-Memory OLTP, Azure SQL platform limits, backup/log-chain readiness, AG seeding edition limits, source lifecycle urgency. Dispatcher with own checks — routes to the security-object and operational-object migration sub-skills, and other specialised skills, for overlap areas |
| [`sqlmigration-security-review`](#sqlmigration-security-review) | `/sqlmigration-security-review`, `/migration-security-review`, `/sql-migration-logins` | Source/target security-object facts pasted as text, capture script output, natural-language description, or directory of exported artifacts | Migration security-object portability audit — 15 checks (J1–J15): login portability (orphaned users, SID mismatch, platform-unsupported login types), permission fidelity (role membership, explicit grants, ownership chains), credentials/secrets, certificates/keys, CMS registrations |
| [`sqlmigration-objects-review`](#sqlmigration-objects-review) | `/sqlmigration-objects-review`, `/migration-objects-review`, `/sql-migration-instance-objects` | Source/target operational-object facts pasted as text, capture script output, natural-language description, or directory of exported artifacts | Migration operational-object portability audit — 16 checks (M1–M16): SQL Agent jobs/operators/alerts/proxies, linked servers, Database Mail, backup devices, custom errors, server triggers, XE sessions, endpoints |
...
| `F1–F36` | `sqlag-review` | AG configuration: AlwaysOn prerequisites, replica design (sync count, session timeout, health check timeout, backup priority, join state), listener architecture (multi-subnet, read-only routing URL/list, MultiSubnetFailover), backup strategy (preference, guard function, log backup scheduling), endpoint security (auth method, cert expiry, RC4 algorithm), distributed AG topology (listener URL, sync link), Basic and Contained AG constraints, operational monitoring, AG database-count scale ceiling | 36 |
...
| `Y1–Y15` | `sqlmigration-review` | Migration compatibility: edition-gated features, version downgrade risk, compatibility level ceiling, collation mismatch, discontinued features, In-Memory OLTP support, Azure SQL Database instance-scoped objects, Windows Auth migration path, Agent availability, backup encryption support, backup chain integrity, recovery model for log-based migration, AG seeding edition limits, source lifecycle urgency, FCI source client-redirect planning | 15 |
...
**Total: 827 checks across all skills.**
