...
| [skills/sqlblocking-review/SKILL.md](skills/sqlblocking-review/SKILL.md) | Live and historical blocking analysis: `sqlblocking-review`. 54 checks (BL1–BL54) — head blocker identification and state classification, chain topology, lock escalation/Sch-M/key-range evidence, transaction and isolation design, blocked process report readiness, per-index and Query Store lock wait history, statistics/lock-partitioning/foreign-key causes, ORM and Azure platform patterns |
...
| [skills/sqlspn-review/SKILL.md](skills/sqlspn-review/SKILL.md) | SPN and Kerberos delegation analysis: `sqlspn-review`. 54 checks (K1–K54) — MSSQLSvc SPN presence, service account binding, AG listener and alias, permissions, Kerberos delegation, AD account sensitivity, Entra ID hybrid and Azure SQL MI Windows Auth, double-hop platform constraints (linked server RBCD, SSISDB, SQL 2017 CU17 floor), Kerberos environment prerequisites (clock skew, token size, encryption types, forest boundary), SQL Server on Linux keytabs, client driver and service state |
...
| [skills/sqlblocking-review/references/check-explanations.md](skills/sqlblocking-review/references/check-explanations.md) | Plain-English explanation of all 54 BL-checks with DMV output examples, the head-blocker state table, background on lock modes and `wait_resource` formats, fix recipes, and Quick Reference table |
...
| [skills/sqlspn-review/references/check-explanations.md](skills/sqlspn-review/references/check-explanations.md) | Plain-English explanation of all 54 K-checks with setspn/AD attribute examples, delegation model tables, Kerberos encryption type bitmask reference, and Quick Reference table |
...
| [skills/sqlblocking-review/references/community-practices.md](skills/sqlblocking-review/references/community-practices.md) | Community practice digest: the four-question method, a tooling map (sp_WhoIsActive, First Responder Kit, sp_HumanEvents, Query Store, index operational stats, Ola Hallengren's WAIT_AT_LOW_PRIORITY parameters), the practitioner rules each check encodes, where popular advice needs qualifying, and the source list |
...
| [PERFORMANCE_TUNING_GUIDE.md](PERFORMANCE_TUNING_GUIDE.md) | Decision guide: which skill to use for which scenario, symptom-based routing, artifact capture how-tos, 904-check ID reference |
| [skills/VERSION_COMPATIBILITY.md](skills/VERSION_COMPATIBILITY.md) | SQL Server version compatibility matrix — which of the 904 checks apply to SQL 2008 R2 through SQL 2022 and Azure SQL; skill-level support matrix; cumulative active check counts per version |
