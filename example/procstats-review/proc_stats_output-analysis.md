...
|------|--------|------|----|-------|----------|------------|-----------|---------|
| 1 | `usp_GetSalesReport` | PROC | SalesDB | 18 | 842.5 | 14,042 | 412,000 | 1,242 |
| 2 | `usp_SyncInventory` | PROC | SalesDB | 4,210 | 31.2 | 2,220 | 8,400 | 1,168 |
| 3 | `usp_SearchCustomers` | PROC | SalesDB | 92,840 | 18.4 | 59 | 12 | 3,713 |
| 4 | `usp_ProcessPayment` | PROC | SalesDB | 1,240 | 8.2 | 1,984 | 2,100 | 865 |
| 5 | `usp_GetOrderHistory` | PROC | SalesDB | 8,420 | 6.1 | 217 | 18,400 | 5,140 |
| 6 | `usp_GeneratePDF` | PROC | SalesDB | 3 | 0.8 | 80,000 | 24,000 | 0.2 |
| 7 | `usp_CleanupSessions` | PROC | SalesDB | 1 | 0.1 | 30,000 | 920,000 | 0.1 |
| 8 | `usp_MonthlyRevenue` | PROC | ReportDB | 4 | 0.0 | 0 | 320,000 | 0.4 |

**Total CPU delta across all objects:** ~922 ms/s
`usp_GetSalesReport` = 842.5 / 922 = **91.4% of total CPU** — extreme concentration (R14: Critical).

---

## Critical Issues

### [C1 — R1] CPU Hotspot — usp_GetSalesReport (842.5 ms/s, 91.4% of total)
...
### [C2 — R6] High Average CPU per Execution — usp_GetSalesReport (14,042 ms avg, 288,400 ms max)
...
### [C3 — R9] Parameter Sniffing Signal — usp_GetSalesReport (max_to_avg ratio: 20.5×)
...
### [W1 — R3] Duration Hotspot — usp_ProcessPayment (avg 39,820 ms elapsed)
...
### [W2 — R8] CPU-Elapsed Skew — usp_ProcessPayment (ratio 0.050 — blocking signal)
...
### [W3 — R7] High Average Reads — usp_GetSalesReport (412,000 reads/execution)
...
### [W4 — R5] Physical I/O — usp_GetSalesReport (84,000 physical reads in interval)
...
### [W5 — R10] High Spills — usp_GetSalesReport (avg 12.4 spills/execution)
...
### [W6 — R12] Chatty Procedure — usp_SearchCustomers (309 executions/sec)
...
- **Observed:** `usp_GetSalesReport` accounts for 91.4% of total CPU delta in the result (842.5 of 922 ms/s). Top 3 procedures = 99.1%.
...
- **Fix:** Prioritization signal — fix C1/C2/C3 on `usp_GetSalesReport` first. All other work is secondary.

### [W8 — R4] Execution Frequency — usp_SearchCustomers (92,840 in interval)
...
### [I1 — R15] Infrequent but Expensive — usp_CleanupSessions (1 exec, 920,000 reads)
...
### [I2 — R11] N+1 Caller Pattern — usp_SearchCustomers (92,840 execs, 12 reads/call, 0 CPU)
...
### [I3 — R13] Plan Instability — usp_GeneratePDF (cache_age_minutes = 8)
...
|----------|--------|----------|--------|
| 1 — Immediately | Add `WITH RECOMPILE` or `OPTION (OPTIMIZE FOR)` to `usp_GetSalesReport` | C3, C2, W5 | 15 min |
| 2 — Today | Run `/sqlplan-review` on `usp_GetSalesReport` plan + generate index DDL | C1, C2, W3 | 1 hr |
| 3 — Today | Investigate blocking in `usp_ProcessPayment` — move external call outside transaction | W1, W2 | 2 hrs |
| 4 — This sprint | Rewrite `usp_SearchCustomers` caller to use TVP batch pattern | W6, W8, I2 | 4 hrs |
| 5 — This sprint | Add covering index on `usp_CleanupSessions` cleanup predicate | I1 | 1 hr |
| 6 — Monitor | Track `plan_handle` for `usp_GeneratePDF` across next 3 intervals | I3 | 30 min |
...
| Check | Result |
|-------|--------|
| R2 — Read Hotspot (per-object) | PASS — no single object > 50% of total reads delta (usp_GetOrderHistory leads at 34%) |
| R4 — Execution Frequency Hotspot | PASS — usp_SearchCustomers at 92,840 is flagged under W8; absolute threshold ≥ 10,000 also fires W8 |
| R5 — Physical I/O Hotspot | PASS — physical_pct for top objects: usp_GetSalesReport 1.1%, usp_CleanupSessions 45.7% (below Critical 50%) |
...
| R15 — Infrequent but Expensive | PASS — usp_CleanupSessions flagged as Info (I1); no additional objects with execs ≤ 5 AND reads ≥ 100K |
