...
|------|--------|------|----|-------|----------|------------|-----------|---------|
| 1 | `usp_GetSalesReport` | PROC | SalesDB | 19 | 981.0 | 15,484 | 428,000 | 2,712 |
| 2 | `usp_SyncInventory` | PROC | SalesDB | 4,350 | 31.8 | 2,192 | 8,520 | 1,240 |

---

## Critical Issues

### [C1 — R1] CPU Hotspot — usp_GetSalesReport (981 ms/s, worsening)
...
### [C2 — R6] High Average CPU — usp_GetSalesReport (15,484 ms at 08:15, up from 6,694 ms)
...
### [W1 — R16] Worsening CPU Trend — usp_GetSalesReport (monotonic, 4 snapshots)
- **Observed:**
...
### [W2 — R18] Read Regression — usp_GetSalesReport (+48% in 15 minutes)
...
### [W3 — R3] Duration Hotspot — usp_GetSalesReport (avg 52,800 ms elapsed at 08:15)
...
### [I1 — R14] Workload Concentration — usp_GetSalesReport dominates CPU
- **Observed:** At 08:15, usp_GetSalesReport accounts for 981/(981+31.8) = **96.8%** of total CPU across both monitored objects.
...
- **Fix:** Prioritization signal — all remediation effort should focus on usp_GetSalesReport.
...
| 3 — Today | Add `OPTION (RECOMPILE)` to usp_GetSalesReport as interim mitigation | W3 | 5 min |
...
| R19 — New High-Cost Entry | PASS — usp_GetSalesReport present in all 4 snapshots; not a new entrant |
| R20 — Plan Instability | PASS — plan_handle identical across all 4 snapshots; no plan change occurred |
| R11 — N+1 Pattern | PASS — usp_GetSalesReport has avg_logical_reads 289K-428K (far above the < 100 reads threshold) |
