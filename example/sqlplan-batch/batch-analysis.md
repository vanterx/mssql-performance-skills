...
> **Input:** `example/sqlplan-batch/plans/` + `example/horrible.sqlplan` — 3 plans
> Run with: `/sqlplan-batch example/sqlplan-batch/plans/ example/horrible.sqlplan`
>
> Plans: `plans/order_report.sqlplan`, `plans/customer_lookup.sqlplan`, `example/horrible.sqlplan`
...
Full analysis: `/sqlplan-review example/horrible.sqlplan`
...
1. **`horrible.sqlplan`** — Fix implicit conversion first (restores cardinality estimates → eliminates spills); then run `/sqlplan-index-advisor example/horrible.sqlplan` for DDL. Full check: `/sqlplan-review example/horrible.sqlplan`.
